...
import { Category, Difficulty, generateCompareNumbers, CompareQuestion } from "@/data/mathQuestions";

type Phase = "setup" | "playing" | "done";

export default function MoreOrLessNumbers() {
  ...
  const nextQ = useCallback(() => {
    setQ(generateCompareNumbers(category, difficulty));
    setFeedback(null);
  }, [category, difficulty]);
  ...
  return (
    <div className="py-6 md:py-10 max-w-2xl mx-auto space-y-6">
      <BackToGames />
      <h1 className="text-2xl font-bold text-foreground">🔢 Więcej czy mniej — Liczby</h1>
      ...
      {phase === "playing" && q && (
        <div className="space-y-4">
          ...
          <Card className={`p-8 text-center transition-colors ${feedback === "correct" ? "border-[hsl(var(--dp-green))]" : feedback === "wrong" ? "border-[hsl(var(--dp-red))]" : ""}`}>
            <div className="flex items-center justify-center gap-6 text-3xl md:text-4xl font-bold text-foreground">
              <span>{q.left}</span>
